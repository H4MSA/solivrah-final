  ...
  return (
    <button
      ...
      className={`flex flex-col items-center justify-center w-16 py-1 transition-all duration-300 ${
        isActive ? 'text-white transform -translate-y-1' : 'text-gray-500'
      }`}
    >
      <div className={`${isActive ? 'transform scale-105' : ''} transition-all duration-300`}>
        {icon}
      </div>
...
export const TabNavigation: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 pb-safe">
      <nav className="flex justify-between items-center mx-4 mb-2 py-1 px-3 bg-black/90 backdrop-blur-xl rounded-lg border border-white/10">
        {navItems.map((item) => (
          <NavItem
            key={item.path}
            icon={item.icon}
