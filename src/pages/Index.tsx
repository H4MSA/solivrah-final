...
const Feature = ({ icon, text }: { icon: React.ReactNode, text: string }) => (
  <div className="flex items-start gap-3 p-3.5 bg-[#1A1A1A]/80 backdrop-blur-md rounded-xl animate-pop-in border border-white/5 hover:border-white/10 transition-all duration-300 transform-gpu hover:scale-[1.02] shadow-lg active:scale-[0.98] touch-manipulation"
      ...
      <div className={`flex-1 flex flex-col justify-center items-center text-center space-y-5 transition-all duration-1000 z-10 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
        ...
        <div className="space-y-5 w-full max-w-xs mx-auto">
          ...
          <div className="flex flex-col w-full gap-3 mt-5">
            <button
              className="flex items-center justify-center gap-2 px-5 py-3.5 bg-white text-black rounded-xl font-semibold text-base transition-all duration-300 hover:bg-[#EEEEEE] active:scale-105 shadow-lg animate-pop-in transform-gpu touch-manipulation"
            ...
            <button
              className="flex items-center justify-center gap-2 px-5 py-3.5 bg-transparent text-white rounded-xl font-medium transition-all duration-300 hover:bg-[#1A1A1A] active:scale-105 border border-white/5 animate-pop-in transform-gpu touch-manipulation"
              onClick={() => navigate("/auth")}
