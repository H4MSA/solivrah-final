...
const StatCard = ({ icon, value, label }: { icon: React.ReactNode, value: number, label: string }) => (
  <div className="flex-1 bg-[#1A1A1A]/80 backdrop-blur-xl border border-white/10 rounded-xl p-3 flex flex-col items-center justify-center gap-1 transition-all hover:bg-[#222222]/80 hover:border-white/15 transform-gpu"
...
const Badge = ({ icon, label, isEarned = true }: { icon: React.ReactNode, label: string, isEarned?: boolean }) => (
...
const ActionButton = ({ icon, label, onClick }: { icon: React.ReactNode, label: string, onClick: () => void }) => (
  <button 
    className="flex-1 bg-[#1A1A1A]/70 backdrop-blur-xl border border-white/10 rounded-xl p-3 flex flex-col items-center justify-center gap-1 hover:bg-[#222222]/80 hover:border-white/15 active:scale-[0.98] transition-all transform-gpu touch-manipulation"
    onClick={onClick}
...
const SettingsItem = ({ icon, label, value, onClick }: { icon: React.ReactNode, label: string, value?: string, onClick: () => void }) => (
  <div 
    className="py-3 px-4 rounded-xl bg-[#1A1A1A]/70 backdrop-blur-xl border border-white/10 flex items-center justify-between hover:bg-[#222222]/80 hover:border-white/15 active:scale-[0.98] transition-all cursor-pointer transform-gpu mb-2 touch-manipulation"
    onClick={onClick}
    style={{ transform: 'translateZ(4px)' }}
  >
...
const Profile = () => {
  const { streak, xp, user, isGuest } = useApp();
